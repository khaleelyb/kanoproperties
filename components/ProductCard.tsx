...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onMessageSeller, isSaved, onToggleSave, onSelectProduct }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl overflow-hidden border border-gray-200 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-50 transition-all duration-300 hover:-translate-y-0.5 flex flex-col">
      {/* Image */}
      <button onClick={() => onSelectProduct(product)} className="relative overflow-hidden bg-gray-100 aspect-[4/3] block">
        {thumbnail ? (
          <img
            src={thumbnail}
            alt={product.title}
        ...
        <button
          ...
          className={`absolute top-2.5 right-2.5 w-8 h-8 rounded-full flex items-center justify-center transition-all shadow-md ${
            isSaved
              ...
              : 'bg-white/90 text-gray-500 hover:bg-white hover:text-blue-600 backdrop-blur-sm'
          }`}
        >
      ...
      <button onClick={() => onSelectProduct(product)} className="flex-1 p-3.5 text-left">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-semibold text-gray-900 text-sm leading-snug line-clamp-2 flex-1">{product.title}</h3>
        </div>
        <span className="inline-block text-xs text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full font-medium mb-2">{product.category}</span>
      ...
      <div className="px-3.5 pb-3.5">
        <button
          ...
          className="w-full py-2 text-sm font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors"
        >
          Contact Agent
        </button>
      </div>
